...
{if !empty($form.address.$blockId.street_address)}
    <tr id="streetAddress_{$blockId}" {if $parseStreetAddress eq 1 && ($action eq 1 || $action eq 2)}class="hiddenElement"{/if}>
        <td colspan="3">
        ...
        {if $parseStreetAddress eq 1 && ($action eq 1 || $action eq 2)}
           {if $action eq 1}<br>{else}&nbsp;&nbsp;{/if}<a href="#" title="{ts}Edit Address Elements{/ts}" onClick="processAddressFields( 'addressElements' , '{$blockId}', 1 );return false;">{ts}Edit Address Elements{/ts}</a>
           {help id="id-edit-street-elements" file="CRM/Contact/Form/Contact.hlp"}
        {/if}
        </td>
    </tr>

    {if $parseStreetAddress eq 1 && $action eq 1}
           <tr id="addressElements_{$blockId}">
               <td>
           ...
           <tr id="addressNameElements_{$blockId}">
               <td colspan="4">
...
function processAddressFields( name, blockId, loadData ) {
  ...
  if ( loadData ) {
      ...
      if (streetAddress === null) streetAddress = '';
       ...
       cj('#addressNameElements_' + blockId).show();
       cj('#streetAddress_' + blockId).hide();
             ...
             cj('#addressNameElements_'+ blockId).hide();
       }
